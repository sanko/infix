 ...
 * 2.  That `arena_alloc` returns non-NULL pointers for valid requests.
 * 3.  That pointers returned by `arena_alloc` are correctly aligned.
 * 4.  That `arena_calloc` returns zero-initialized memory.
...
TEST {
    ...
    subtest("Arena Allocator Core Functionality") {
        ...
        ok(arena != NULL && arena->buffer != NULL, "arena_create succeeds");
        if (!arena) {
        ...
        ok(p1 != NULL, "arena_alloc returns a valid pointer");

        // 3. Test Alignment
        // Allocate a single byte to misalign the offset, then allocate an aligned type.
        (void)arena_alloc(arena, 1, 1);
        uint64_t * aligned_ptr = arena_alloc(arena, sizeof(uint64_t), _Alignof(uint64_t));
        ok(aligned_ptr != NULL, "arena_alloc for aligned type succeeds");
        ok(((uintptr_t)aligned_ptr % _Alignof(uint64_t)) == 0, "Pointer is correctly aligned");

        // 4. Test arena_calloc
        char * zeroed_ptr = arena_calloc(arena, 1, 32, 1);
        bool is_zeroed = true;
        for (int i = 0; i < 32; ++i) {
            if (zeroed_ptr[i] != 0) {
                is_zeroed = false;
        ...
        ok(is_zeroed, "arena_calloc returns zero-initialized memory");

        // 5. Test Out-of-Memory Condition
        void * p_fail = arena_alloc(arena, 2048, 1);  // Request more than is available
        ok(p_fail == NULL, "arena_alloc returns NULL when out of memory");
