...
TEST {
    ...
    subtest("Passing and returning pointers") {
        ...
        const char * unbound_result = NULL;
        ((infix_cif_func)infix_forward_get_code(unbound_t))((void *)find_char_in_string, &unbound_result, args);
        ok(unbound_result && strcmp(unbound_result, "FFI World!") == 0, "Unbound call correct");

        // Bound
        infix_forward_t * bound_t = NULL;
        ok(infix_forward_create_bound_manual(&bound_t, ret_type, arg_types, 2, 2, (void *)find_char_in_string) ==
               INFIX_SUCCESS,
           "Bound created");
        const char * bound_result = NULL;
    ...
    subtest("Modifying data via pointer arguments") {
        ...
        infix_forward_t * unbound_t = NULL;
        ok(infix_forward_create_manual(&unbound_t, ret_type, arg_types, 2, 2) == INFIX_SUCCESS, "Unbound created");
        ((infix_cif_func)infix_forward_get_code(unbound_t))((void *)modify_data_via_pointers, NULL, args);
        ...
        // Bound
        infix_forward_t * bound_t = NULL;
        ok(infix_forward_create_bound_manual(&bound_t, ret_type, arg_types, 2, 2, (void *)modify_data_via_pointers) ==
               INFIX_SUCCESS,
           "Bound created");
        ((infix_bound_cif_func)infix_forward_get_code(bound_t))(NULL, args);
    ...
    subtest("Passing NULL pointers") {
        ...
        ((infix_cif_func)infix_forward_get_code(unbound_t))((void *)check_if_null, &res_null, args_null);
        ok(res_null == true, "Unbound NULL correct");
        ((infix_cif_func)infix_forward_get_code(unbound_t))((void *)check_if_null, &res_valid, args_valid);
        ok(res_valid == false, "Unbound non-NULL correct");

        // Bound
        infix_forward_t * bound_t = NULL;
        ok(infix_forward_create_bound_manual(&bound_t, ret_type, arg_types, 1, 1, (void *)check_if_null) ==
               INFIX_SUCCESS,
           "Bound created");
        ((infix_bound_cif_func)infix_forward_get_code(bound_t))(&res_null, args_null);
        ok(res_null == true, "Bound NULL correct");
        ((infix_bound_cif_func)infix_forward_get_code(bound_t))(&res_valid, args_valid);
        ok(res_valid == false, "Bound non-NULL correct");
