...
- Fixed a libFuzzer timeout in the RISC-V 64-bit classifier caused by zero-sized aggregate chains. `rv64_flatten_all_recursive` iterated every element of an array regardless of element size; a zero-sized element never advances the offset, so chains like `struct { array[127][127][127][127][127][0] }` exploded exponentially (~33M iterations) without ever producing a leaf to trip the `RV_MAX_FLATTENED_FIELDS` leaf-table guard. The flattener now has a recursion-depth limit (`RV_MAX_FLATTEN_DEPTH`), null member/element guards, leaf-table capacity checks before descending, and flattens zero-sized elements once at the starting offset. Added a `TARGET_ABI_CLASSIFIER` regression target in `t/850_regression_cases.c` that exercises the active ABI's forward/reverse classifiers on the crashing input.
