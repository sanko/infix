...
- Fixed ARM64 forward trampoline GPR marshalling dropping the trailing members of 5/6/7-byte structs passed by value. `generate_forward_argument_moves_arm64` emitted a 32-bit load (`ldr w`) for every GPR-passed aggregate unless its size was exactly 8, so a struct like `{ uint16_t arr[2]; uint16_t x; }` loaded only the first four bytes. Sizes 5-7 now load a full 64-bit register; sizes 1-4 keep their exact-byte loads.
...
- Fixed unbounded memory growth in the global forward trampoline cache. Every unique signature permanently retained its JIT-compiled handle (executable code, deep-copied type graph, and signature string) with no eviction, so an application. The cache is now bounded to `INFIX_CACHE_MAX_ENTRIES` (1024) entries with least-recently-used eviction; evicted handles are destroyed when their reference count drops to zero.
- Fixed RISC-V classifier failing to deal with zero-sized aggregate chains. A zero-sized element never advances the offset so chains like `struct { array[127][127][127][127][127][0] }` exploded exponentially (~33M iterations) without ever producing a leaf to trip the `RV_MAX_FLATTENED_FIELDS` leaf-table guard. The flattener now has a recursion-depth limit (`RV_MAX_FLATTEN_DEPTH`), null member/element guards, leaf-table capacity checks before descending, and flattens zero-sized elements once at the starting offset.
...
[Unreleased]: https://github.com/sanko/infix/compare/v0.2.1...HEAD
[0.2.1]: https://github.com/sanko/infix/compare/v0.2.0...v0.2.1
