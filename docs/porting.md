...
*   **For Windows ARM64:** You must use a two-entry `RUNTIME_FUNCTION` table (the second entry is a sentinel) and the `UNWIND_INFO_ARM64` header must include the `FunctionLength` field.
