 ...
 * @brief Common definitions for the AArch64 (ARM64) architecture.
 *
 * @details This header defines enums for the general-purpose registers (GPRs) and
...
typedef enum {
    ...
    X8_REG,      ///< Indirect result location register (holds the address for returning large structs) / Volatile.
    ...
    X16_REG,     ///< Intra-Procedure-call scratch register (IP0) / Volatile. Can be modified by linkers.
    X17_REG,     ///< Intra-Procedure-call scratch register (IP1) / Volatile. Can be modified by linkers.
    X18_REG,  ///< Platform Register (reserved, usage is platform-specific, e.g., thread pointer) / May be callee-saved.
              ///< Avoid use.
    ...
    X30_LR_REG,   ///< Link Register (LR), holds the return address / Volatile.
    SP_REG = 31,  ///< Stack Pointer (SP). In some instructions, encoding 31 refers to the Zero Register (XZR/WZR).
} arm64_gpr;

/**
 * @enum arm64_vpr
 * @brief Enumerates the ARM64 Floating-Point/SIMD (NEON) registers, also known as V-registers.
 *
...
typedef enum {
    ...
    V8_REG,      ///< Callee-saved (Note: only the lower 64 bits must be preserved by a callee).
