...
/**
 * @internal
 * @brief Emits a 2-byte or 3-byte VEX prefix for an AVX instruction.
 * @details The VEX prefix is a modern instruction prefix that replaces the older
 *          REX, SIMD, and operand-size prefixes for AVX instructions. It encodes
 *          register extensions, operand size, and other features in a more
 *          compact format. This function automatically chooses the most compact
 *          (2-byte) form when possible.
 *
 *          For a definitive guide, see the Intel 64 and IA-32 Architectures
 *          Software Developer's Manual, Volume 2, Chapter 2 ("Instruction Format").
 *
 * @param buf The code buffer.
 * @param r Inverted R bit. `true` if the ModR/M `reg` field uses an extended register (XMM8-15/R8-15).
 * @param x Inverted X bit. `true` if the SIB `index` field uses an extended register.
 * @param b Inverted B bit. `true` if the ModR/M `r/m` or SIB `base` field uses an extended register.
 * @param m The `m-mmmm` field, typically `0b00001` for instructions from the `0F` map.
 * @param w The W bit. `true` for 64-bit operand size, `false` otherwise.
 * @param v The `vvvv` field, an extra register operand specifier, inverted.
 * @param l The L bit. `true` for 256-bit vectors (YMM), `false` for 128-bit (XMM).
 * @param p The `pp` field, specifying the SIMD prefix (00=None, 01=66, 10=F3, 11=F2).
 */
static void emit_vex_prefix(
    ...
    // A value of `true` (1) for r, x, or b means the corresponding bit in the prefix is 0.
    if (!b && !x && m == 1 && w == 0) {
        // 2-byte VEX prefix (C5)
        emit_byte(buf, 0xC5);
        uint8_t byte2 = 0;
        byte2 |= (!r) << 7;        // Inverted R bit
        byte2 |= (~v & 0xF) << 3;  // `vvvv` field (inverted)
        byte2 |= (l & 1) << 2;
        byte2 |= (p & 3);
        emit_byte(buf, byte2);
    }
        ...
        uint8_t byte2 = 0;
        byte2 |= (!r) << 7;  // Inverted R bit
        byte2 |= (!x) << 6;  // Inverted X bit
        byte2 |= (!b) << 5;  // Inverted B bit
        byte2 |= (m & 7);
        emit_byte(buf, byte2);
        uint8_t byte3 = 0;
        byte3 |= (w & 1) << 7;
        byte3 |= (~v & 0xF) << 3;  // `vvvv` field (inverted)
        byte3 |= (l & 1) << 2;
        byte3 |= (p & 3);
        emit_byte(buf, byte3);
    }
...
/**
 * @brief Emits `vmovupd ymm, [src_base + offset]` (load 256-bit AVX vector from memory).
 * @details This instruction loads a 256-bit unaligned packed double-precision
 *          floating-point value from memory into a YMM register.
 *
 *          - **Instruction format:** `VEX.256.66.0F.WIG 10 /r`
 *
 * @param buf The code buffer.
 * @param dest The destination YMM register (specified as an x64_xmm enum).
 * @param src_base The base GPR for the memory address.
 * @param offset The 32-bit signed offset from the base register.
 */
void emit_vmovupd_ymm_mem(code_buffer * buf, x64_xmm dest, x64_gpr src_base, int32_t offset) {
    ...
    // R,B are for extended regs. m-mmmm=01 (from 0F map). W is ignored. L=1 (256-bit). p=1 (from 66 prefix).
...
/**
 * @brief Emits `vmovupd [dest_base + offset], ymm` (store 256-bit AVX vector to memory).
 * @details This instruction stores a 256-bit unaligned packed double-precision
 *          floating-point value from a YMM register to memory.
 *
 *          - **Instruction format:** `VEX.256.66.0F.WIG 11 /r`
 *
 * @param buf The code buffer.
 * @param dest_base The base GPR for the memory address.
 * @param offset The 32-bit signed offset from the base register.
 * @param src The source YMM register (specified as an x64_xmm enum).
 */
void emit_vmovupd_mem_ymm(code_buffer * buf, x64_gpr dest_base, int32_t offset, x64_xmm src) {
    // VEX prefix fields for vmovupd m256, ymm:
    // R,B are for extended regs. m-mmmm=01 (from 0F map). W is ignored. L=1 (256-bit). p=1 (from 66 prefix).
