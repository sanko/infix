 ...
 *
 * @internal
 * This file provides the implementation for debugging helper functions, such as
 * `infix_dump_hex`. The content is conditionally compiled based on the
 * `INFIX_DEBUG_ENABLED` macro.
 *
 ...
 * use the double_tap test framework.
 * @endinternal
 */
...
/*
 * This section conditionally includes and defines the `note()` macro.
 * If the double_tap test harness is fully implemented (`DBLTAP_IMPLEMENTATION` is defined),
 * we use its `note()` macro for integrated test logging. Otherwise, we define a simple
 * `printf`-based fallback. This allows debug builds to function even outside the
 * test suite.
 */
...
void infix_dump_hex(const void * data, size_t size, const char * title) {
    ...
    for (size_t i = 0; i < size; i += 16) {
        ...
        // The `goto` is used here as a simple error-handling mechanism. If `snprintf`
        // fails or indicates the buffer is full, we jump to the `print_line` label
        // to output whatever portion of the line has been formatted so far.
