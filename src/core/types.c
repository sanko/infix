...
c23_nodiscard infix_status infix_type_create_struct(infix_arena_t * arena,
    ...
    for (size_t i = 0; i < num_members; ++i) {
        infix_struct_member * member = &members[i];
 ...
 *          like `__attribute__((packed))` or `#pragma pack(1)`. All memory is
 ...
 *
 * @note **Memory Ownership:** All memory for the created `infix_type` and its metadata
 *       is owned by the provided arena.
 */
