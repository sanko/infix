...
INFIX_API infix_status emit_math_mov_imm(emit_context_t * ctx, emit_register_t dest, uint64_t imm) {
    ...
    if (ctx->arch == EMIT_ARCH_X86_64) {
        ...
        (void)emit_emit_u8(ctx, 0xB8 | (dest & 0x07));
...
INFIX_API infix_status emit_math_movq_gpr_xmm(emit_context_t * ctx, emit_register_t gpr_dest, emit_register_t xmm_src) {
    if (ctx->arch == EMIT_ARCH_X86_64) {
        ...
        (void)emit_emit_u8(ctx, 0x66);
        emit_x86_rex(ctx, true, EMIT_REG_NEEDS_REX(xmm_src), false, EMIT_REG_NEEDS_REX(gpr_dest));
        (void)emit_emit_u8(ctx, 0x0F);
        (void)emit_emit_u8(ctx, 0x7E);
        (void)emit_emit_u8(ctx, 0xC0 | ((xmm_src & 0x07) << 3) | (gpr_dest & 0x07));
    }
    return INFIX_SUCCESS;
}
...
INFIX_API infix_status emit_math_add_imm(emit_context_t * ctx, emit_register_t dest, int32_t imm) {
    ...
    if (ctx->arch == EMIT_ARCH_X86_64) {
        emit_x86_rex(ctx, true, false, false, EMIT_REG_NEEDS_REX(dest));
        (void)emit_emit_u8(ctx, 0x81);
        (void)emit_emit_u8(ctx, 0xC0 | (dest & 0x07));
...
INFIX_API infix_status emit_math_sub_imm(emit_context_t * ctx, emit_register_t dest, int32_t imm) {
    ...
    if (ctx->arch == EMIT_ARCH_X86_64) {
        emit_x86_rex(ctx, true, false, false, EMIT_REG_NEEDS_REX(dest));
        (void)emit_emit_u8(ctx, 0x81);
        (void)emit_emit_u8(ctx, 0xE8 | (dest & 0x07));
        (void)emit_emit_u32(ctx, (uint32_t)imm);
    }
    return INFIX_SUCCESS;
}
...
INFIX_API infix_status emit_math_mul(emit_context_t * ctx, emit_register_t src) {
    ...
    if (ctx->arch == EMIT_ARCH_X86_64) {
        ...
        (void)emit_emit_u8(ctx, 0xF7);
        (void)emit_emit_u8(ctx, 0xE0 | (src & 0x07));
    }
    return INFIX_SUCCESS;
}
...
INFIX_API infix_status emit_math_imul_imm(emit_context_t * ctx, emit_register_t dest, int32_t imm) {
    ...
    if (ctx->arch == EMIT_ARCH_X86_64) {
        ...
        if (imm >= -128 && imm <= 127) {
            (void)emit_emit_u8(ctx, 0x6B);
            (void)emit_emit_u8(ctx, 0xC0 | ((dest & 0x07) << 3) | (dest & 0x07));
            (void)emit_emit_u8(ctx, (uint8_t)imm);
        } else {
            (void)emit_emit_u8(ctx, 0x69);
            (void)emit_emit_u8(ctx, 0xC0 | ((dest & 0x07) << 3) | (dest & 0x07));
            (void)emit_emit_u32(ctx, (uint32_t)imm);
        }
    }
...
INFIX_API infix_status emit_math_not(emit_context_t * ctx, emit_register_t reg) {
    ...
    if (ctx->arch == EMIT_ARCH_X86_64) {
        ...
        (void)emit_emit_u8(ctx, 0xF7);
        (void)emit_emit_u8(ctx, 0xD0 | (reg & 0x07));
    }
    return INFIX_SUCCESS;
}
...
INFIX_API infix_status emit_math_neg(emit_context_t * ctx, emit_register_t reg) {
    ...
    if (ctx->arch == EMIT_ARCH_X86_64) {
        emit_x86_rex(ctx, true, false, false, EMIT_REG_NEEDS_REX(reg));
        (void)emit_emit_u8(ctx, 0xF7);
        (void)emit_emit_u8(ctx, 0xD8 | (reg & 0x07));
    }
    return INFIX_SUCCESS;
}
...
INFIX_API infix_status emit_math_shl(emit_context_t * ctx, emit_register_t dest, emit_register_t src) {
    ...
    if (ctx->arch == EMIT_ARCH_X86_64) {
        ...
        emit_x86_rex(ctx, true, false, false, EMIT_REG_NEEDS_REX(dest));
        (void)emit_emit_u8(ctx, 0xD3);
        (void)emit_emit_u8(ctx, 0xE0 | (dest & 0x07));
    }
    return INFIX_SUCCESS;
}
...
INFIX_API infix_status emit_math_cmp(emit_context_t * ctx, emit_register_t a, emit_register_t b) {
    ...
    if (ctx->arch == EMIT_ARCH_X86_64) {
        ...
        (void)emit_emit_u8(ctx, 0x39);
        (void)emit_emit_u8(ctx, 0xC0 | ((b & 0x07) << 3) | (a & 0x07));
    }
    return INFIX_SUCCESS;
}
...
INFIX_API infix_status emit_math_cmp_imm(emit_context_t * ctx, emit_register_t reg, int32_t imm) {
    ...
    if (ctx->arch == EMIT_ARCH_X86_64) {
        emit_x86_rex(ctx, true, false, false, EMIT_REG_NEEDS_REX(reg));
        (void)emit_emit_u8(ctx, 0x81);
        (void)emit_emit_u8(ctx, 0xF8 | (reg & 0x07));
        (void)emit_emit_u32(ctx, (uint32_t)imm);
    }
    return INFIX_SUCCESS;
}
...
INFIX_API infix_status emit_math_jmp_cc(emit_context_t * ctx, emit_cc_t cc, const char * label) {
    ...
    if (ctx->arch == EMIT_ARCH_X86_64) {
        uint64_t jump_offset = ctx->current_section->size;
        (void)emit_emit_u8(ctx, 0x0F);
        (void)emit_emit_u8(ctx, x86_jcc_opcodes[cc]);
        (void)emit_emit_u32(ctx, 0);
...
INFIX_API infix_status emit_math_prologue(emit_context_t * ctx) {
    if (ctx->arch == EMIT_ARCH_X86_64) {
        (void)emit_emit_u8(ctx, 0x55);
        (void)emit_emit_u8(ctx, 0x48);
        (void)emit_emit_u8(ctx, 0x8B);
        (void)emit_emit_u8(ctx, 0xEC);
    }
    return INFIX_SUCCESS;
}
...
INFIX_API infix_status emit_math_epilogue(emit_context_t * ctx) {
    if (ctx->arch == EMIT_ARCH_X86_64) {
        (void)emit_emit_u8(ctx, 0xC9);
        (void)emit_emit_u8(ctx, 0xC3);
    }
    return INFIX_SUCCESS;
}

INFIX_API infix_status emit_math_ret(emit_context_t * ctx) {
    if (ctx->arch == EMIT_ARCH_X86_64) (void)emit_emit_u8(ctx, 0xC3);
    return INFIX_SUCCESS;
}
...
INFIX_API infix_status emit_math_push(emit_context_t * ctx, emit_register_t reg) {
    if (ctx->arch == EMIT_ARCH_X86_64) {
        ...
        (void)emit_emit_u8(ctx, 0x50 | (reg & 0x07));
    }
    return INFIX_SUCCESS;
}
...
INFIX_API infix_status emit_math_pop(emit_context_t * ctx, emit_register_t reg) {
    if (ctx->arch == EMIT_ARCH_X86_64) {
        emit_x86_rex(ctx, false, false, false, EMIT_REG_NEEDS_REX(reg));
        (void)emit_emit_u8(ctx, 0x58 | (reg & 0x07));
    }
    return INFIX_SUCCESS;
}
...
INFIX_API infix_status emit_math_load_reg(emit_context_t * ctx, emit_register_t dest, emit_register_t base, int32_t offset) {
    if (ctx->arch == EMIT_ARCH_X86_64) {
        ...
        (void)emit_emit_u8(ctx, 0x8B);
        (void)emit_emit_u8(ctx, mod | ((dest & 0x07) << 3) | (base & 0x07));
        if ((base & 0x07) == 4) (void)emit_emit_u8(ctx, 0x24);
        if (mod == 0x40) (void)emit_emit_u8(ctx, (uint8_t)offset);
        else if (mod == 0x80) (void)emit_emit_u32(ctx, (uint32_t)offset);
    }
    return INFIX_SUCCESS;
}
...
INFIX_API infix_status emit_math_store_reg(emit_context_t * ctx, emit_register_t base, int32_t offset, emit_register_t src) {
    if (ctx->arch == EMIT_ARCH_X86_64) {
        ...
        (void)emit_emit_u8(ctx, 0x89);
        (void)emit_emit_u8(ctx, mod | ((src & 0x07) << 3) | (base & 0x07));
        if ((base & 0x07) == 4) (void)emit_emit_u8(ctx, 0x24);
        if (mod == 0x40) (void)emit_emit_u8(ctx, (uint8_t)offset);
        else if (mod == 0x80) (void)emit_emit_u32(ctx, (uint32_t)offset);
    }
...
INFIX_API infix_status emit_math_load_sym(emit_context_t * ctx, emit_register_t dest, const char * sym) {
    if (ctx->arch == EMIT_ARCH_X86_64) {
        ...
        (void)emit_emit_u8(ctx, 0x8B);
        (void)emit_emit_u8(ctx, 0x05 | ((dest & 0x07) << 3));
        (void)emit_emit_u32(ctx, 0);
...
INFIX_API infix_status emit_math_store_sym(emit_context_t * ctx, const char * sym, emit_register_t src) {
    if (ctx->arch == EMIT_ARCH_X86_64) {
        ...
        (void)emit_emit_u8(ctx, 0x89);
        (void)emit_emit_u8(ctx, 0x05 | ((src & 0x07) << 3));
        (void)emit_emit_u32(ctx, 0);
...
INFIX_API infix_status emit_math_movsd_reg(emit_context_t * ctx, emit_register_t dest, emit_register_t src) {
    if (ctx->arch == EMIT_ARCH_X86_64) {
        (void)emit_emit_u8(ctx, 0xF2);
        emit_x86_rex(ctx, false, EMIT_REG_NEEDS_REX(dest), false, EMIT_REG_NEEDS_REX(src));
        (void)emit_emit_u8(ctx, 0x0F);
        (void)emit_emit_u8(ctx, 0x10);
        (void)emit_emit_u8(ctx, 0xC0 | ((dest & 0x07) << 3) | (src & 0x07));
    }
    return INFIX_SUCCESS;
}
