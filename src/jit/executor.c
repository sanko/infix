...
static void initialize_macos_apis(void) {
    ...
    // Look for JIT helpers in the default search scope (libSystem)
...
static bool has_jit_entitlement(void) {
    ...
    // Hardened JIT requires both the entitlement AND the pthread toggle API.
...
c23_nodiscard infix_executable_t infix_executable_alloc(size_t size) {
    ...
    exec.rw_ptr = code;
    exec.rx_ptr = code;
#elif defined(INFIX_OS_MACOS) || defined(INFIX_OS_ANDROID) || defined(INFIX_OS_OPENBSD)
    ...
    if (code != MAP_FAILED && g_use_secure_jit_path) {
        // MAP_JIT memory is non-writable by default. Switch this thread to Write mode.
    ...
    exec.rx_ptr = code;
#else
    // Dual-mapping POSIX platforms (Linux, FreeBSD, DragonFly).
    // Create two separate views of the same memory.
...
c23_nodiscard bool infix_executable_make_executable(infix_executable_t * exec) {
...
#elif defined(INFIX_OS_MACOS)
    // Use Apple-specific API if available (required for Apple Silicon correctness)
    ...
    if (g_use_secure_jit_path && g_macos_apis.pthread_jit_write_protect_np) {
        // Toggle this thread to Execute mode (1).
