...
int main() {
    // Load the User32 library and get the function pointer.
    ...
    // Describe the function signature using the high-level string API.
    ...
    // Generate the trampoline from the signature.
    infix_forward_t * trampoline = NULL;
    infix_forward_create(&trampoline, signature);

    // Prepare arguments. Windows uses UTF-16 for wide strings, so we use the L"" prefix.
