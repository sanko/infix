...
- [x] **Use Guard Pages for Freed Trampolines**
    ...
    *   **Goal:** The `ffi_executable_free` function is updated to use `mprotect`/`VirtualProtect`. A new test is created that frees a trampoline and then calls its old function pointer, asserting that the program safely terminates.
...
- [x] **Add `long double` Support**
    ...
    *   **Goal:** `long double` is a recognized `ffi_type` and passes correctly in forward and reverse calls on all supported platforms (System V, Windows/GCC, AArch64).

- [x] **Read-Only Callback Contexts (RELRO for Callbacks)**
    *   **Context:** The `ffi_reverse_trampoline_t` struct contains function pointers that could be targeted by memory corruption attacks to hijack control flow.
    ...
    *   **Goal:** A new optional API (`generate_hardened_reverse_trampoline`) is added. Any attempt to write to a hardened context struct will cause an immediate segmentation fault, preventing the attack.
...
- [ ] **Direct System Call Interface**
    ...
    *   **Idea:** Create a new `generate_syscall_trampoline` API that emits assembly to load registers and execute the `syscall` instruction.
...
- [ ] **Add Exception Handling Boundary**
    ...
    *   **Idea:** Create a `generate_safe_forward_trampoline` function where the JIT code wraps the native call in a `try...catch` or `__try...__except` block.
    *   **Goal:** If a native function throws an exception, the trampoline catches it, returns `FFI_ERROR_NATIVE_EXCEPTION`, and the program does not crash.
...
- [ ] **Add Bitfield Support in Structs**
    ...
    *   **Idea:** Extend the `ffi_type` system to describe bitfields and implement the packing/unpacking logic for each ABI.
...
- [ ] **Add Type System Builder API**
    *   **Context:** The current API for creating complex structs is verbose and requires manual memory management for the member array.
    *   **Idea:** Create a fluent builder pattern API (e.g., `ffi_struct_builder_*` functions) to simplify type creation.
    *   **Goal:** A user can define a complex struct without manually allocating or managing the `ffi_struct_member` array, reducing boilerplate and potential for errors.
...
- [ ] **Decouple Debug Logging from `double_tap.h`**
    *   **Context:** The library's internal `FFI_DEBUG_PRINTF` macro should not be tied to the test framework's `note()` function, which improves modularity.
    *   **Idea:** Refactor `FFI_DEBUG_PRINTF` in `utility.h` to use `fprintf(stderr, ...)` directly.
...
- [ ] **Add Support for Advanced C Types (`_Complex` and SIMD)**
    ...
    *   **Idea:** Extend the `ffi_type` system with new categories or primitives for these types. Implement the corresponding ABI classification and marshalling logic in each backend. For example, `_Complex` types are often passed as if they were a two-element struct of floats/doubles.
    *   **Goal:** A user can create an `ffi_type` for `double _Complex` and successfully call a function that uses it, with the library handling the ABI rules correctly on all supported platforms.
